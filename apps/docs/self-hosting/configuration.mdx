...
<ParamField path="SMTP_REJECT_UNAUTHORIZED" default="true">
  ...
  <Note>This option is only available from v4.4.0 and later.</Note>
</ParamField>

<ParamField path="SMTP_TLS_SERVERNAME">
  Sets the hostname used for TLS certificate validation. Required when `SMTP_HOST` is set to an IP address or a hostname that only exists in `/etc/hosts` (not resolvable via DNS), as nodemailer cannot determine the correct servername automatically in those cases. See [nodemailer TLS options](https://nodemailer.com/smtp#tls-options) for more details.

  <Note>This option is only available from v4.8.0 and later.</Note>
</ParamField>

<ParamField path="SMTP_TLS_ENABLED">
  <Warning>Deprecated: Use `SMTP_REJECT_UNAUTHORIZED=false` to disable certificate validation</Warning>
...
<ParamField path="OIDC_ISSUER_URL">
  <Warning>Deprecated from v4.5.0: This option is no longer used.</Warning>
...
Storage configuration is optional but required to enable features like user avatar uploads.
For self-hosted deployments, [MinIO](https://min.io/) is the recommended S3-compatible storage solution. Alternatively, you can use cloud services like [AWS S3](https://aws.amazon.com/s3/), [DigitalOcean Spaces](https://www.digitalocean.com/products/spaces), or [Cloudflare R2](https://www.cloudflare.com/developer-platform/r2/).
